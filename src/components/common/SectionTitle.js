const SectionTitle = ({ title, index, isRight, isTransparent }) => {
  return (
    <div className={`h-[100px] md:h-[230px] w-full font-Montserrat ${isTransparent ? 'bg-transparent' : 'bg-black'}`}>
      <div className="relative w-full h-full font-montserrat">
        <div className={`absolute bottom-0 ${isRight ? 'right-0' : 'left-10 md:left-[180px]'}`}>
          <h5 className="text-[50px] md:text-[148px] text-[#FFFFFF33] leading-none uppercase">{title}</h5>
        </div>
        <div
       ...
       `}
        >
          <div className="flex items-end h-full">
            <div className="my-auto w-[50px] md:w-[70px] border-[2px] border-[#B0985A]"></div>
